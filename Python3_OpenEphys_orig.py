...
def loadFolder(folderpath,**kwargs):
    ...
    if 'channels' in kwargs.keys():
        filelist = ['101_CH'+x+'.continuous' for x in map(str,kwargs['channels'])]
    else:
...
def loadFolderToArray(folderpath, channels='all', dtype=float, 
    source='101', recording=None, start_record=None, stop_record=None,
    verbose=True):
...
def align_timestamps(data_np, ls, gap_idx, trial_length):
    
    """
    ls: list of records; each trail length = record*1024/30000
    data_np: data from one channel
    gap_idx: 1st index after the offset of recording
    ...
    tmp = []
    start = 0
    for ii in range(len(ls[gap_idx:])):
        end = ls[gap_idx+ii]*samples_per_record + start

        if ls[gap_idx+ii]>100:        # this ensures only long trials are considered
            tmp.append(data_np[start:end])         
            start = end
        else:
            start = end               # to account for wrong length trials in the middle of acquisition 

    df = pd.DataFrame(tmp)
    
    df = df.iloc[:,:trial_length]
    df = df.fillna(0.0)
    data_al = np.reshape(np.asarray(df), (1, (df.index[-1]+1)*trial_length))
    return data_al    
...
def loadSpikes(filepath):
    ...
    while f.tell() < os.fstat(f.fileno()).st_size:
        ...
        for ch in range(numChannels):
            spikes[currentSpike,:,ch] = (np.float64(wv[ch])-32768)/(gain[currentSpike,ch]/1010)
...
def _get_sorted_channels(folderpath, recording=None):
    ...
    if recording is None:
        ...
        regex_pattern = '%s_CH(\d+)%s.continuous' % ('101', recording_s)
...
def get_filelist(folderpath, source='101', channels='all', recording=None):
